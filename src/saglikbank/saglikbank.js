...
function SaglikBank() {
    ...
    function f_encodeTurkish(_chunk) {
        return iconv.decode(new Buffer(_chunk, 'binary'), 'ISO-8859-9');
    }

    function f_rss(_firstX, _url) {
    ...
    function f_isSessionUp(_cookieString) {
    ...
    function f_postCookie(_cookie) {
        console.log("---------- F_POST_COOKIE -------------");
        console.log(_cookie);
        var defer = db.redis.dbQ.Q.defer();
        db.dbQ.zadd("COOKIES", (new Date()).getTime(), JSON.stringify(_cookie))
            .then(function (r) {
...
//TODO: Sadece bir şartname varsa tamam ama zeyilname ya da birden fazla dosya için kontrol edilecek
    ...
    function f_getBids(_firstX, _url) {
        var defer = db.redis.dbQ.Q.defer();
